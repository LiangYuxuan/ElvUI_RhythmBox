...
function SMB:HandleBlizzardButtons()
    ...
    if E.db.RhythmBox.MinimapButtons.MoveTracker and not _G.MinimapCluster.TrackingFrame.Button.SMB then
        -- _G.MinimapCluster.TrackingFrame.Show = nil

        _G.MinimapCluster.TrackingFrame.Button:Show()
        SMB:SetTemplate(_G.MinimapCluster.TrackingFrame.Button)

        _G.MinimapCluster.TrackingFrame.Button:SetParent(self.Bar)
        ...
        _G.MinimapCluster.TrackingFrame.Button:HookScript('OnEnter', function(self)
            _G.MinimapCluster.TrackingFrame.Button:SetBackdropBorderColor(unpack(SMB.ClassColor))
            if SMB.Bar:IsShown() then
                UIFrameFadeIn(SMB.Bar, 0.2, SMB.Bar:GetAlpha(), 1)
            end
        end)
        _G.MinimapCluster.TrackingFrame.Button:HookScript('OnLeave', function(self)
            SMB:SetTemplate(_G.MinimapCluster.TrackingFrame.Button)
            if SMB.Bar:IsShown() and E.db.RhythmBox.MinimapButtons.BarMouseOver then
                UIFrameFadeOut(SMB.Bar, 0.2, SMB.Bar:GetAlpha(), 0)
            end
        ...
        if E.db.RhythmBox.MinimapButtons.Shadows then
            _G.MinimapCluster.TrackingFrame.Button:CreateShadow()
        end

        tinsert(self.Buttons, _G.MinimapCluster.TrackingFrame.Button)
    end
