...
const updateAutoGeneratedSingle = (key: string, content: string) => {
    ...
    files.forEach((file) => {
        ...
        console.info(new Date().toISOString(), `[INFO]: Handling ${relateivePath}`);
        ...
        if (newFileContent !== fileContent) {
            console.info(new Date().toISOString(), `[INFO]: Updating ${relateivePath}`);
            fs.writeFileSync(file, newFileContent, 'utf-8');
        }
    });
};

const updateAutoGenerated = (key: string, content: string | Map<string, string>) => {
    if (typeof content === 'string') {
        updateAutoGeneratedSingle(key, content);
    } else {
        content.forEach((value, subKey) => {
            const fullKey = `${key}${subKey}`;
            updateAutoGeneratedSingle(fullKey, value);
        });
    }
};
