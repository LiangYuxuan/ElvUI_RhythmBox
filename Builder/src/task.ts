...
import { CASCClient, WDCReader } from '@rhyster/wow-casc-dbc';
...
interface Task {
    ...
    handler: (datas: WDCReader[]) => Promise<string> | string,
}
...
export const executeTasks = async () => {
    ...
    for (let i = 0; i < softTasks.length; i += 1) {
        const task = softTasks[i];
        const { key, fileDataIDs, handler } = task;

        console.log(new Date().toISOString(), `[INFO]: Executing task ${(i + 1).toString()} ${key}`);
        ...
        if (!fileDataIDs) {
            ...
            if (forceTasks.includes(task)) {
                ...
                // eslint-disable-next-line no-await-in-loop
                const text = await handler(dbFiles.map((dbFile) => dbFile.reader));
